    ...
    for (int i = 0; i < count; i++) {
        ...
        if (Dart_IsInteger(value)) {
            ...
            CheckSqlError(statement->db,
                          sqlite3_bind_text(statement->stmt, i + 1, result, strlen(result), SQLITE_TRANSIENT));
            ...
            CheckDartError(Dart_TypedDataReleaseData(value));
            CheckSqlError(statement->db, sqlite3_bind_blob(statement->stmt, i + 1, result, length, sqlite3_free));
        } else {
...
DART_FUNCTION(Step) {
    ...
    while (true) {
        ...
        switch (status) {
            ...
            case SQLITE_ROW: DART_RETURN(get_last_row(statement));
            default:
